...
class DisplayRestaurantHours extends StatelessWidget {
  ...
  Widget build(BuildContext) {
    return Column(
      ...
      children: open
          .map((time) =>
              Text('${dayOfTheWeek[time.day]} ${processingTime(time)}', style: TextStyle(height: 1.32),))
          .toList(),
    );
  }
}
...
String processingTime(OpenHours dayInfo){
  ...
  final startTimeWithColon = startTime.substring(0,2) + ':' + startTime.substring(2,4) + 'AM';
  final endTimeWithColon = endTime.substring(0,2) + ':' + endTime.substring(2,4) + 'PM';
  //if(dayInfo.isOvernight)
  return '$startTimeWithColon - $endTimeWithColon';
}

/*
String? processingHours(OpenHours dayInfo) {
  switch (dayInfo.day) {
    case 0:
      {
        return 'Sunday';
